...
def _active_edges_single_path(
    ...
    if use_graph_primitive:
        ...
        for i in range(n):
            ...
            # endpoints are those with degree 1
            endpoint = degree == 1
            is_endpoint.append(endpoint)

            # rank logic for connectivity and acyclicity
            smaller_rank_count = count_true(
                [
                    is_active_edge[e] & (rank[j] < rank[i])
                    for j, e in graph.incident_edges[i]
                ]
            ...
            solver.ensure(
                is_passed[i].then(smaller_rank_count == is_root[i].cond(0, 1))
            )
            solver.ensure(is_root[i].then(is_passed[i] & endpoint))

            for j, e in graph.incident_edges[i]:
                if i < j:
                    solver.ensure(is_active_edge[e].then(rank[i] != rank[j]))
